...
class _FavouritesScreenState extends ConsumerState<FavouritesScreen> {
    ...
    return Scaffold(
      body: SafeArea(
        child: Column(
          ...
          children: [
            _buildHeader(context, favouritesState),
            if (favouritesState.movies.isNotEmpty) _buildSortBar(context),
            const SizedBox(height: 8),
            Expanded(
              child: _buildContent(context, favouritesState, genresAsync),
            ),
          ],
        ),
      ),
  ...
  Widget _buildHeader(BuildContext context, FavouritesState state) {
    return Padding(
              ...
              children: [
                ...
                Text(
                  state.movies.isEmpty
                      ? 'No movies yet'
                      : '${state.movies.length} movie${state.movies.length != 1 ? 's' : ''} saved',
  ...
  Widget _buildContent(
    ...
    if (favouritesState.isLoading && favouritesState.movies.isEmpty) {
      return const MovieGridShimmer();
    }

    if (favouritesState.error != null && favouritesState.movies.isEmpty) {
      return AppErrorWidget(
        message: favouritesState.error!,
        onRetry: () => ref.read(favouritesProvider.notifier).loadFavourites(),
      );
    }

    if (favouritesState.movies.isEmpty) {
      return EmptyStateWidget(
        title: 'No Favourites Yet',
        message: 'Movies you mark as favourite will appear here.\nStart exploring and save movies you love!',
        icon: Icons.favorite_border_rounded,
      );
    }

    final genresMap = genresAsync.valueOrNull ?? {};
    final sortedMovies = _sortMovies(favouritesState.movies);

    return RefreshIndicator(
      onRefresh: () => ref.read(favouritesProvider.notifier).loadFavourites(),
      color: AppColors.accentGold,
              ...
              child: ScaleAnimation(
                child: FadeInAnimation(
                  child: Dismissible(
                    ...
                    confirmDismiss: (direction) async {
                      return await showDialog(
                        ...
                        builder: (context) => AlertDialog(
                          ...
                          content: Text('Remove "${movie.title}" from your favourites?'),
                          actions: [
                            TextButton(
                    ...
                    onDismissed: (_) {
                      ref.read(favouritesProvider.notifier).removeFromFavourites(movie.id);
                    },
                    child: MovieCard(
                      movie: movie,
                      genresMap: genresMap,
